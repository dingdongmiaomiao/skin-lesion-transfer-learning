...
本阶段进行迁移学习微调策略对比。根据工序2结果，后续实验采用 AdamW + ReduceLROnPlateau 作为默认训练策略。
...
其中“仅训练分类头”结果来自工序2的 AdamW + ReduceLROnPlateau 实验。
...
| 解冻最后10层 | 0.855562 | 15 | 4,853,761 |
| 全模型微调 | 0.885590 | 7 | 11,177,025 |

### 结果分析

实验结果显示，随着可训练层数增加，模型验证集 AUC 明显提升。解冻最后10层相比仅训练分类头有一定提升，而全模型微调提升最明显，最佳验证 AUC 达到 0.885590。

这说明 ISIC 皮肤镜图像与 ImageNet 自然图像之间存在领域差异，仅训练分类头难以充分适应医学图像分类任务。全模型微调能够使低层和高层特征共同适应皮肤病灶图像，因此取得了最佳性能。

不过，全模型微调在后期出现验证 Loss 波动，说明其存在一定过拟合风险。因此后续最终评估和可解释性分析采用 full_finetune 在第 7 个 epoch 保存的最佳模型。

## 记录 9：最终模型评估结果

### 实验设置

本阶段选取工序3中表现最好的 full_finetune 最佳模型作为最终模型，在验证集上进行评估。评估内容包括：

- ROC 曲线；
- 混淆矩阵；
- AUC、Accuracy、Precision、Recall、Specificity、F1 等分类指标；
...
最终模型在验证集上的 AUC 为 0.8856，说明模型具有较好的良恶性排序判别能力。默认阈值 0.5 下，模型召回 117 个恶性样本中的 90 个；使用 Youden 阈值后，恶性召回数提升到 102 个，漏诊数从 27 降低到 15。

由于验证集中恶性样本数量远少于良性样本，类别分布极不平衡，因此 Precision 和 F1 分数较低。对于黑色素瘤筛查任务，较高的 Recall / Sensitivity 具有重要意义，但误报数量较多也说明模型仍需要结合医生复核或进一步优化。

## 记录 10：Grad-CAM 可解释性分析

### 实验设置

本阶段使用最终 full_finetune 最佳模型生成 Grad-CAM 热力图，用于分析模型在皮肤病灶良恶性分类时关注的图像区域。
...
对于预测正确的恶性样本，Grad-CAM 热力图大多集中在病灶主体或其周围异常纹理区域，说明模型能够关注与恶性判别相关的视觉特征。

对于漏诊的恶性样本，热力图常出现偏离病灶主体、集中在图像边缘或背景区域的情况，说明模型未能充分捕捉关键病灶特征。

对于误报的良性样本，热力图主要集中在颜色较深、纹理复杂或边界不规则区域，说明模型可能将部分良性病灶的复杂视觉表现误认为恶性特征。

对于预测正确的良性样本，由于本次 Grad-CAM 主要解释恶性类别方向，因此热力响应整体较弱。

### 结论

Grad-CAM 结果说明最终模型具有一定可解释性，能够在部分恶性样本中关注病灶相关区域。但误报和漏诊样本也表明模型仍可能受到局部颜色、纹理和图像边缘等因素影响，需要在后续工作中进一步优化。

## 记录 11：t-SNE 特征可视化

### 实验设置

本阶段使用最终 full_finetune 最佳模型提取验证集图像的深度特征，并使用 t-SNE 进行二维可视化。特征提取位置为分类头前一层，即 ResNet-18 全局平均池化后的特征向量。

为降低计算量和噪声，t-SNE 前先使用 PCA 将特征降至 50 维。随后分别绘制两种可视化结果：
...
整体抽样验证集包含 2500 个样本，其中良性样本 2383 个，恶性样本 117 个。由于类别极度不平衡，图中良性样本占主导，恶性样本虽然在部分区域有一定聚集，但整体与良性样本仍存在明显重叠。

类别平衡抽样包含 200 个样本，其中良性样本 100 个，恶性样本 100 个。在平衡抽样图中，两类样本呈现一定分离趋势，恶性样本更多分布在图的上方和右侧，良性样本更多集中在下方区域。

### 结果分析

t-SNE 结果说明最终模型提取的深度特征具有一定判别能力，但良性与恶性样本在特征空间中并非完全分离。这与混淆矩阵和 Grad-CAM 结果一致，说明模型虽然能够较好地区分部分恶性样本，但仍存在误报和漏诊问题。